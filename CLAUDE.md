...
**Document lifecycle (the core domain model)** — `Quotation`, `DeliveryOrder`, `Invoice` in `prisma/schema.prisma` all share one shape and one rule: a document is freely editable (client, line items, notes, date) while `issuedAt IS NULL`. Clicking "Issue" on the owning Project's hub page is the one-way lock: in a single `$transaction` it assigns a sequential number via `lib/numbering.ts` (`nextDocumentNumber`, shared across all three types — per-doc-type-per-year counter in `DocumentCounter`, format `Q-2026-0001` / `DO-2026-0001` / `INV-2026-0001`), sets `issuedAt`, and (for Quotation/Invoice, not DeliveryOrder) advances `status`. After that, content is locked forever for Quotation and DeliveryOrder; only status transitions and re-downloads are allowed. Each doc type's `actions.ts` (`app/(app)/quotations/actions.ts`, `.../delivery-orders/actions.ts`, `.../invoices/actions.ts`) has a `saveX` (create/update), `issueX` (the numbering/lock transaction), and `setXStatus` (an explicit `ALLOWED_TRANSITIONS` map — extend that map, don't ad-hoc a new transition) — copy this three-function shape for any change here. Status semantics differ per type: Quotation issuance moves `DRAFT → SENT`; DeliveryOrder issuance stays `DRAFT` (a DO can be locked/numbered while still "not yet physically delivered" — `DELIVERED` is a separate manual action); Invoice issuance moves `DRAFT → UNPAID`, and `UNPAID ⇄ PAID` afterwards is **not** gated by the lock (it's metadata, not content). **Invoice is the one exception to the content-lock**: `saveInvoice` stays callable after `issuedAt` is set — content (line items, discount, notes, number, etc.) can be edited any time, re-downloading the PDF always reflects current data. The one thing that still locks Invoice content is **any** `Receipt` existing against it (`invoice.receipts.length > 0` — an Invoice can have several, one per payment installment received, not just one; see `lib/money.ts`'s `invoiceUncreditedAmount`), since a Receipt's amount and printed description are fixed at issuance and would silently disagree with a later edit. This is deliberately keyed off receipt-existence, not `status === "PAID"` — `PAID ⇄ UNPAID` is an allowed, UI-exposed transition, so a status-only lock could be released with one click ("Mark Unpaid") even while already-issued receipts reference the current line items. A Receipt's own `description`/`notes` text is likewise snapshotted onto the `Receipt` row at issuance (`app/(app)/receipts/actions.ts`) rather than recomputed live in the PDF route, so re-downloading an older partial receipt after a later one exists can't retroactively change what it says.
...
**PDF rendering**: `lib/pdf/DocumentPdf.tsx` is one `@react-pdf/renderer` component, prop-driven by doc type label/footer, rendered server-side via `renderToBuffer` in each `app/api/documents/<type>/[id]/pdf/route.ts` — that's the only place it's used; there is no live in-app preview. Changing document layout means editing this one component, not five.

**Project hub**: `Project` (`prisma/schema.prisma`) wraps discussion notes plus exactly one Quotation→DeliveryOrder→Invoice→Receipt chain (`Quotation.projectId`, nullable+unique). `app/(app)/projects/[id]/page.tsx` is the single page for driving that whole chain — every workflow action (issue, accept/reject, convert, mark delivered/paid, create/receive a Deposit Invoice, issue a Receipt) lives there as a button calling the same per-doc-type Server Actions described below, plus a Download link per issued document straight to its `/api/documents/.../pdf` route. The five old `quotations/actions.ts` (etc.) still hold all the actual mutation logic — they just now redirect back to `/projects/${projectId}` (resolved via a relation walk when the acting document isn't the Quotation itself) instead of to their own document's page. The individual `[id]/page.tsx` for Quotation/DeliveryOrder/Invoice now exist only for **editing content**, not workflow actions, and redirect to the owning Project the moment that's no longer true (Quotation/DeliveryOrder: once `issuedAt` is set; Invoice: once any `Receipt` exists, matching its content-lock exception above).
